...
const App = () => {
  ...
  useEffect(() => {
    ...
    !localStorage.getItem("city") && localStorage.setItem("city", "andijan city")
    !localStorage.getItem("is_entered") && localStorage.setItem("is_entered", "false")
  ...
  useEffect(() => {
    if (userSignIn) {
      const get_f = async () => {
        try {
          JSON.parse(localStorage.getItem("likedProducts"))?.map(async (item) => {
            await create_favorites(item.product, localStorage.getItem("userId"))
          })
  ...
  if (!is_online) {
    ...
    return (
      <div className={`${is_found ? "sm:w-[1450px]" : "w-full "} m-auto `}>
        ...
        <div className={`${is_found ? "sm:w-[1450px]" : "w-full"} m-auto overflow-hidden`}>
          <div
            className={`flex flex-col justify-between ${is_found ? (is_another_nav ? "h-[calc(106.9vh-100px)] sm:h-[calc(118vh)]" : "h-[calc(106.9vh-100px)] sm:h-[calc(119.5vh-100px)]") : "h-full"} w-[100%]`}
            style={{
              ...customScrollbar,
              overflowY: "auto",
              scrollbarWidth: "none",
          ...
          >
            <div style={transitionStyles}>
              <Routes location={displayLocation}>
                ...
                <Route
                  ...
                  element={
                    localStorage.getItem("is_entered") == "true" ? (
                      ...
                      <Enter_language lang={lang} set_lang={set_lang} set_is_found={set_is_found} />
                    )
                  }
                />
                <Route
                ...
                <Route
                  ...
                  element={
                    localStorage.getItem("is_entered") == "true" ? (
                      ...
                      <Enter_borrow lang={lang} set_city={set_city} city={city} set_is_found={set_is_found} />
                    )
                  }
                />
                <Route
                ...
                />
                <Route path="/payment/success" element={<PaymentSuccess set_is_PM={set_is_PM} />} />
                <Route
                  path="/product/*"
                  element={
                    <Suspense fallback={<div>Loading...</div>}>
                      <Product set_basket={set_basket} basket={basket} lang={lang} userSignIn={userSignIn} />
                    </Suspense>
                  }
                />
                <Route
                ...
                />
                <Route path="/installment" element={is_SI ? <Installment /> : <Navigate to="/" />} />
                <Route
                  path="/terms"
                  element={formalization_open && basket.length ? <Terms lang={lang} /> : <Navigate to="/" />}
                />
                <Route path="*" element={<Not_found lang={lang} set_is_found={set_is_found} />} />
                <Route
                  path="/register"
                  element={<Register lang={lang} set_is_found={set_is_found} setUserSignIn={setUserSignIn} />}
                />
                <Route
                  path="/login"
                  element={<Log_in lang={lang} set_is_found={set_is_found} setUserSignIn={setUserSignIn} />}
                />
        ...
        <Payment_success lang={lang} is_modal_open={is_PM} set_is_modal_open={set_is_PM} />
      </div>
    )
  }
}
