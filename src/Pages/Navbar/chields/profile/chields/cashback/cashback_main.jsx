import React from "react";

const Cashback_main = () => {
  return (
    <div className="w-[40%] mx-auto bg-white rounded-lg">
      <div className="flex justify-between mb-4 font-inter font-[600] text-[20px] leading-[22px] text-black">
        <div>185.000 UZS</div>
        <div>185.000 UZS</div>
      </div>

      <div className="flex justify-between text-sm mb-6 font-inter font-[600] text-[15px] leading-[22px] text-black">
        <div>Umumiy keshbek</div>
        <div>Yechilgan</div>
      </div>

      <div className="mb-6">
        <div className="font-inter font-[600] text-[15px] leading-[22px] text-black">
          63224636-sonli buyurtma
        </div>
      </div>

      <div className="space-y-2 mb-4">
        <div className="flex justify-between font-inter font-[500] text-[15px] leading-[22px] text-black">
          <div>Yetkazish sanasi</div>
          <div>Juma 10 yanvar</div>
        </div>
        <div className="flex justify-between font-inter font-[500] text-[15px] leading-[22px] text-black">
          <div>Rasmiylashtirish sanasi</div>
          <div>Juma 10 yanvar</div>
        </div>
      </div>

      <div className="space-y-2 mb-6 font-inter font-[600] text-[15px] leading-[22px] text-black">
        <div className="flex justify-between">
          <div>1 dona maxsulot</div>
          <div>126.650 so'm</div>
        </div>
        <div className="flex justify-between text-[#12CF38]">
          <div>Keshbek</div>
          <div>12.000 so'm</div>
        </div>
      </div>

      <hr className="border-[#0000008C] mb-6" />

      <div className="mb-6">
        <div className="font-inter font-[600] text-[15px] leading-[22px] text-black">
          63224636-sonli buyurtma
        </div>
      </div>

      <div className="space-y-2 mb-4">
        <div className="flex justify-between font-inter font-[500] text-[15px] leading-[22px] text-black">
          <div>Yetkazish sanasi</div>
          <div>Juma 10 yanvar</div>
        </div>
        <div className="flex justify-between font-inter font-[500] text-[15px] leading-[22px] text-black">
          <div>Rasmiylashtirish sanasi</div>
          <div>Juma 10 yanvar</div>
        </div>
      </div>

      <div className="space-y-2 mb-6 font-inter font-[600] text-[15px] leading-[22px] text-black">
        <div className="flex justify-between">
          <div>1 dona maxsulot</div>
          <div>126.650 so'm</div>
        </div>
        <div className="flex justify-between text-[#12CF38]">
          <div>Keshbek</div>
          <div>12.000 so'm</div>
        </div>
      </div>
    </div>
  );
};

export default Cashback_main;
